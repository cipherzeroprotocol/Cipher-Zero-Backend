...
Metadata.prototype.setMetadata = function(_metadata) {
  ...
  if (this.files && this._encodedMetadata) {
    ...
    this._encodedMetadata = Buffer.alloc(Buffer.byteLength(bencode.encode(_metadata)), bencode.encode(_metadata));
    LOGGER.debug(this._encodedMetadata.length);
  ...
  if (!this.infoHash) {
    ...
    var digest = crypto.createHash('sha1')
      .update(bencode.encode(_metadata), 'ascii')
      .digest();
    var infoHash = Buffer.alloc(Buffer.byteLength(digest), digest, 'binary');
