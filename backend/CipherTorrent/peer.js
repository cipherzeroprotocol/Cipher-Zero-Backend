...
var BITTORRENT_HEADER = Buffer.alloc( Buffer.byteLength(protocolHeaderStr), protocolHeaderStr, "binary");
...
Peer.prototype.sendExtendedMessage = function(type, data) {
  ...
  if (code !== undefined) {
    ...
    var codeAsBuffer = Buffer.alloc(1);
    codeAsBuffer[0] = code;

    LOGGER.debug('Peer [%s] extended request code = %j', this.getIdentifier(), codeAsBuffer[0]);

    var payload = Buffer.alloc(Buffer.byteLength(bencode.encode(data)), bencode.encode(data));
    ...
    if (self.toSend.length > 0) {
      ...
      self.torrent.requestChunk(index, begin, length, function(err, data) {
        if (err) {
          ...
          if (data) {
            var msgBuffer = Buffer.alloc(8+data.length);
